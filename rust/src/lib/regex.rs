...
unsafe extern "C" fn push_fail_stack(
    ...
    let ref mut fresh61 = (*((*fs).stack).offset(num as isize)).regs;
    *fresh61 = malloc(
...
unsafe extern "C" fn re_copy_regs(
    ...
    while (i as libc::c_ulong) < (*regs).num_regs {
        let ref mut fresh62 = *((*regs).end).offset(i as isize);
        *fresh62 = -(1 as libc::c_int) as regoff_t;
        *((*regs).start).offset(i as isize) = *fresh62;
        i += 1;
        i;
    }
